...
# keypoints 좌표 추출을 위해 새롭게 정의한 함수
def keypoints_extractor(canvas, candidate, subset):
    stickwidth = 4
    limbSeq = [
        [2, 3],
        [2, 6],
    ...
    keypoints = [(-1, -1) for _ in range(18)]
    allkeypoints = {}
    for n in range(len(subset)):
        for i in range(18):
            index = int(subset[n][i])
